...
class RewardWrapper:
    ...
    def step(self, action, render=False):
        ...
        right_active = np.sum(near_right)
        front_active = np.sum(near_front)
        left_active  = np.sum(near_left)

        any_signal = (right_active + front_active + left_active + ir) > 0

        # ===== DIRECTIONAL LOGIC =====

        if any_signal and not self.env.enable_push:

            # ---- FRONT → move forward ----
            if front_active > 0:
                if action == "FW":
                    r += self.w_forward * front_active
                else:
                    r -= 1.0

            # ---- LEFT → turn right ----
            if left_active > 0:
                if action in ["R22", "R45"]:
                    r += self.w_turn * left_active
                else:
                    r -= 0.5

            # ---- RIGHT → turn left ----
            if right_active > 0:
                if action in ["L22", "L45"]:
                    r += self.w_turn * right_active
                else:
                    r -= 0.5
        ...
        # ===== SMALL PENALTY FOR DOING NOTHING USEFUL =====
        if not any_signal and not self.env.enable_push:
            r -= 0.5
